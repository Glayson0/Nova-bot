...
class BusSchedule:
    ...
    def __repr__ (self) -> str:
        return (
            f"Onibus das {self.next_departure} às {self.next_return}"
            if self.next_departure is not None else r"Acabaram os ônibus por hoje\!"
    )

# Horários dos ônibus

# Dia útil
weekdayBusDeparture = deque((
...
# Dia não-útil
nonWorkingDayBusDeparture = deque((
...
def create_bus_schedule(
    ...
    next_bus = ( departure.popleft(), return_.popleft() )
    is_first_bus = True
    is_last_bus = False
    ...
    for _ in range(MINUTES_IN_A_DAY):
        ...
        # BUG o return_ deveria ser sempre o mais próximo possível do departure
        if time_as_str == next_bus[0]:
            if len(departure):
                next_bus = ( departure.popleft(), return_.popleft() )
        ...
        bus_schedule[time_as_str] = BusSchedule(
            *next_bus, tuple(departure), is_weekend, is_first_bus, is_last_bus
        )
        is_first_bus = False
